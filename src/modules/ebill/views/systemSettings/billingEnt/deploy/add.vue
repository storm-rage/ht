<template>
  <div id="bn-add">
    <div class="zj-p-x-20 zj-p-y-20"><!-- class="title-item" -->
      <h4 class="text-center">新增开单业务参数</h4>
      <i style="color:#f56c6c">*</i>&nbsp;开单业务方：
      <el-select
        filterable
        allow-create
        ...
        <el-option
          v-for="item in dictionary.buyerEntList"
          :key="item.id"
          :value="item.id"
          :label="item.name"
    ...
    <el-form :model="formModel" :rules="formRules" label-width="210px" ref="form" :class="pageType === 'detail' ? 'bn-deploy_detail' : ''">
      <el-collapse ref="elCollapse" v-model="activeNames" class="zjcoll-noSelect" style="border-bottom: none">
        <el-collapse-item name="business" :disabled="true">
          <template slot="title">
          ...
          <el-form-item label="核心企业开单额度模式：" prop="buyerLimitModel" v-if="isSupportOpenQuota">
        ...
        <el-collapse-item name="expire" :disabled="true" v-if="isSupportExpireClear">
          <template slot="title">
            <h4 class="orderTitle">
          ...
          <el-form-item label-width="310px" label=" " class="zj-m-b-0" v-if="formModel.clearModel === '3'">
            <el-form-item label-width="130px" label="清算方企业名称：" class="zj-m-b-0" prop="clearEntName">
              <el-select v-model="formModel.clearEntName" filterable :popper-append-to-body="false" @change="clearEntChange">
                <el-option v-for="item in dictionary.clearEnts" :key="item.entId" :label="item.entName" :value="item.entName"/>
              </el-select>
            </el-form-item>
    ...
    <el-row class="w85 zj-p-l-300">
      <zj-button class="pass" @click="handleAdd" :api="zjBtn.addEbBuyerParam">保存</zj-button>
...
<script>
  export default {
    data(){
      return {
        ...
        zjControl:{
          ...
          addEbBuyerParam:this.$api.billingEnt.addEbBuyerParam,//修改
        },
        dictionary:{},
        formModel:{
          entId: '',
        ...
        formRules:{
          billModel:[
            { required:true,message:'请选择开单模式',trigger: 'blur'},
          ],
          buyerLimitModel:[
            { required:true,message:'请选择开单额度模式',trigger: 'blur'},
          ],
          minOpenDays:[
            { required:true,message:'请填写开单最小期限',trigger: 'blur'},
            { validator:this.minOpenDaysValidate ,trigger: 'blur'}
          ],
          maxOpenDays:[
            { required:true,message:'请填写开单最长期限',trigger: 'blur'},
            { validator:this.maxOpenDaysValidate ,trigger: 'blur'}
          ],
          openExchangeRate:[
            { required:true,message:'请输入折扣率默认值',trigger: 'blur'},
          ],
          clearModel:[
            { required:false,message:'请选择清算模式',trigger: 'blur'},
          ],
          clearEntName:[
            { required:true,message:'请选择清算方企业',trigger: 'blur'},
          ],
        },
        billModelList:[],//开单模式
    ...
    methods:{
      ...
      changeEntName (id) {
        this.formModel.entId = id
        this.formModel.entName = this.dictionary.buyerEntList.find(item => {return id === item.id}).name
      },
      //开单模式
      ...
      clearEntChange(name){
        if(!name){return}
        this.dictionary.clearEnts.map(item => {
      ...
      handleAdd(){
        ...
        this.$refs.form.validate(boo => {
          ...
          this.zjControl.addEbBuyerParam(params).then(ret => {
            this.$messageBox({
              type: 'success',
              content: ret.msg || '新增成功',
              messageResolve: ()=> {
                this.goParent()
              }
            })
