<template>
  <div class="zj-p-t-20 zj-p-b-20">
    <el-row class="ta-c">
      <label class="jc20" v-if="isEdit">银行对接详情</label>
    </el-row>
    <el-form ref="form" class="w500px zj_20px-auto-0" :model="form" label-width="140px">
      <el-form-item label="所属项目：" prop="projectName" class="" :rules="[{ required: false, message: '请输入项目名称', trigger: 'change' }]">
        <el-input v-model="form.projectName" maxlength="200" :disabled="!isPlatform" disabled/>
      </el-form-item>
      <el-form-item label="银行类型：" prop="projectName" class="" :rules="[{ required: false, message: '请输入项目名称', trigger: 'change' }]">

        <el-input v-model="form.bankType" v-text="typeMap(dictionarys.bankType,form.bankType)" maxlength="200" :disabled="!isPlatform"  disabled />
      </el-form-item>
      <el-form-item label="银行名称：" prop="projectName" class=""　 :rules="[{ required: false, message: '请输入项目名称', trigger: 'change' }]">
        <el-input v-model="form.bankName" maxlength="200" :disabled="!isPlatform" disabled/>
      </el-form-item>
      <el-form-item label="说明：" prop="remark" class="" :rules="[{ required: false, message: '请输入项目说明', trigger: 'change' }]">
        <el-input v-model="form.remark" maxlength="200" :disabled="!isPlatform" disabled/>
      </el-form-item>
      <div style="box-shadow: 0 0 5px rgba(172,172,172,.4);padding: 5px 0 5px 10px;margin: 10px 0">接口参数</div>
      <template v-if="form.bankType === 'ABC'">
        <el-form-item label="产品编码：" prop="productId" class="required" :rules="[{ required: true, message: '请输入产品编码', trigger: 'change' }]">
          <el-input v-model="form.productId" maxlength="200" :disabled="!isPlatform" disabled/>
        </el-form-item>
        <el-form-item label="平台编码：" prop="sysCode" class="required" :rules="[{ required: true, message: '请输入平台编码', trigger: 'change' }]">
          <el-input v-model="form.sysCode" maxlength="200" :disabled="!isPlatform" disabled/>
        </el-form-item>
        <el-form-item label="商圈编码：" prop="busiGroupCode" class="required" :rules="[{ required: true, message: '请输入商圈编码', trigger: 'change' }]">
          <el-input v-model="form.busiGroupCode" maxlength="200" :disabled="!isPlatform" disabled/>
        </el-form-item>
      </template>
      <template v-if="form.bankType === 'CITIC'">
        <el-form-item label="所属机构：" prop="branch" class="required" :rules="[{ required: true, message: '请输入所属机构', trigger: 'change' }]">
          <el-input v-model="form.branch" maxlength="200" :disabled="!isPlatform" disabled/>
        ...
        <el-form-item label="会员确认中心：" prop="mngNode" class="required" :rules="[{ required: true, message: '请输入会员确认中心', trigger: 'change' }]">
          <el-input v-model="form.mngNode" maxlength="200" :disabled="!isPlatform" disabled/>
          <p class="explainText">说明：用于赋值接口字段-mngNode</p>
        </el-form-item>
      </template>
<!--      提交-->
      <el-form-item>
<!--        <zj-button  status="primary" @click="submitForm" v-if="isEdit" :api="zjBtn.updatePubProject">保存</zj-button>-->
<!--        <zj-button  status="primary" @click="submitForm" v-else :api="zjBtn.addPubProject">保存</zj-button>-->
...
export default {
  ...
  created() {
    ...
    if (this.row.id) {
      ...
      this.isEdit = true;
      const {id} = this.row;
      this.zjControl.bankDockSetGetDetail({id}).then(ret => {
        if (ret.success && ret.data) {
          ...
          // });
          this.form.bankName = ret.data.bankName;
          this.form.busiGroupCode = ret.data.busiGroupCode;
          this.form.remark = ret.data.remark;
          this.form.sysCode = ret.data.sysCode;
          this.form.mngNode = ret.data.mngNode;
  ...
  methods:{
    ...
    submitForm () {
      ...
      //   }
      // });
      this.zjControl.bankDockSetUpdate(
        {
