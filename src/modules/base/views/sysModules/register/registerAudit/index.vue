    ...
    <zj-list-layout ref="dd">
      <template slot="searchForm">
        <el-form ref="searchForm" :model="searchForm" class="search-form">
          ...
          <el-form-item label="注册申请日期：" class="col-center">
            <zj-date-range-picker class="searchFormDate" :startDate.sync="searchForm.applyDatetimeStart" :endDate.sync="searchForm.applyDatetimeEnd" />
          </el-form-item>
          ...
          <el-form-item label="申请流水号：">
            <el-input v-model="searchForm.serialNoLike" placeholder="" @keyup.enter.native="enterSearch" />
          </el-form-item>
      ...
      <zj-table ref="searchTable" :params="searchForm" :api="zjControl.tableApi">
        <zj-table-column field="serialNo" title="申请流水号">
          <template v-slot="{ row }">
            <span class="table-elbill-code" @click="goChild('registerAuditApplyDetail', row)">{{ row.serialNo }}</span>
          </template>
        </zj-table-column>
        ...
        <zj-table-column title="操作" fixed="right">
          <template v-slot="{ row }">
            <zj-button type="text" @click="goChild('registerAuditApplyAudit', row)" v-if="row.registerState === '2' || row.registerState === '3'">审核</zj-button>
            <zj-button type="text" :api="zjBtn.getCertUserInfo" @click="certificate(row)" v-else-if="row.registerState === '4'">发证</zj-button>
            <span v-else>—</span>
          </template>
        </zj-table-column>
      </zj-table>
