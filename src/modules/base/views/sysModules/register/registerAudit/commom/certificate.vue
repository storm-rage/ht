...
>
  ...
  <div class="bodyDiv" style="width: 100%">
    <zj-table ref="searchTable" class="zj-search-table" style="min-width: 80%!important;" :dataList="tableList">
      <zj-table-column field="loginName" title="用户名" width="125"/>
      <zj-table-column field="userName" title="姓名" width="125"/>
      <zj-table-column field="certType" title="证件类型" width="125" :formatter="(obj) => typeMap(directory.legalCertTypeList,obj.cellValue)"/>
      <zj-table-column field="certNo" title="证件号码" width="125"/>
      <zj-table-column field="roleName" title="操作角色" width="125"/>
      <zj-table-column field="issueCertState" title="发证书" width="85" v-if="cBtn.certKey">
        <template v-slot="{row}">
          <zj-button type="text" v-if="row.issueCertState === '0'" @click="makeKey(row)">
            {{operatCertType === '1' ? '制key' : operatCertType === '2' ? '绑定云证书' : ''}}
          </zj-button>
          <span v-else>
            {{operatCertType === '1' ? '已制key' : operatCertType === '2' ? '已绑定云证书' : ''}}
          </span>
        </template>
      </zj-table-column>
    </zj-table>
  ...
  <el-row slot="footer" class="dialog-footer">
    <zj-button status="primary" @click="submitCert" :api="cBtn.submitEntIssuedCert">发证完成</zj-button>
  </el-row>
</el-dialog>
</template>
