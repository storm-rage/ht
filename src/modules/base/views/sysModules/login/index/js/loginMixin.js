...
import {genMockMenu} from '@/router/mockMenuUtils'
export default {
    name: 'login',
    components: {
        LoginHeader,LoginFooter,LoginEditPassword,frozenDialog
    },
    data(){
        return{
            //  表单校验规则
    ...
    methods:{
        ...
        clearForm () {
          ...
          this.isOneLogin = '0' // 隐藏手机验证码输入框
        },
      //保存相关信息
      saveInfo(loginRes){
        // 设置项目列表
        this.$store.dispatch('project/setProjectList', loginRes.projectList);
        if (loginRes.projectList&&loginRes.projectList.length) {
          this.$store.dispatch('project/setProjectInfo', loginRes.projectList[0]);
        }
        const menuList = loginRes.resList.concat(genMockMenu())
        // 保存菜单树
        ...
        inputBlur(e) {
          ...
          if(e.currentTarget.name === 'loginName'){
            if(this.updateName !== this.userData.loginName){
              ...
              this.isOneLogin = '1' // 隐藏手机验证码填入框
            }
          }
        },
        inputInitialize() { this.$refs.userForm.$el.elements.forEach(item => { if (item.type !== 'button') { item.style.backgroundColor = 'rgba(155,155,155,0.1)' } })
        },
        //回车登录
        keyDown(e){
            if(e.keyCode == 13){
    ...
    mounted() {
        ...
        this.inputInitialize() // 初始化input样式
