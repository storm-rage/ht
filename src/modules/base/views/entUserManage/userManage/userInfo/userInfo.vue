    ...
    <div class="zj-search-response">
      ...
      >
        ...
        <!-- <zj-table-column
          field="userState"
          title="当前状态"
          :formatter="(obj) => typeMap(uDictionary.userState, obj.cellValue)"
        /> -->
        <!-- <zj-table-column title="联网核查" v-if="uDictionary.isIdCheck === '1'">
          <template v-slot="{ row }">
            <zj-button
              type="text"
              @click="verification(row)"
              :api="uBtn.uValidateMobileRealName"
              >联网核查</zj-button
            >
          </template>
        </zj-table-column> -->
        <zj-table-column title="操作" fixed="right">
          <template v-slot="{ row }">
            <zj-button
              type="text"
              @click="goChild('userManageMag', row)"
              :api="uBtn.uUpdateUser"
              >管理</zj-button
            >
            <!-- <zj-button
              type="text"
              @click="makeCertKey(row)"
              v-if="row.keyFlag"
              :api="uBtn.uMakeCertKey"
              >制key</zj-button
            > -->
            <zj-button
              type="text"
              @click="goChild('userManageDetail', row)"
